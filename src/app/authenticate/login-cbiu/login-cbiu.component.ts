...
@Component({
  selector: 'app-login-cbiu',
  templateUrl: './login-cbiu.component.html',
  styleUrls: ['./login-cbiu.component.scss']
})
export class LoginCbiuComponent implements OnInit {
      ...
      this.api.getToken(
        ...
        .subscribe((res) => {
          ...
          if (res.status == 'success') {
            localStorage.setItem('APPLOGO', 'assets/images/cbiu-logo.png');
            localStorage.setItem('UNITTITLE', 'CBIU');
            localStorage.setItem('LOGINPAGE', '/authenticate/login/cbiu');
            localStorage.setItem('Unit_name', 'CBIU');
                ...
                this.api.getAPI(environment.API_URL + "access/permissions?user_role_id=" + res.role_center[0].user_role.id + '&process_id=' + res.process_id).subscribe((res1) => {
                  if (res1.status == environment.SUCCESS_CODE) {
                    if (res1.data.length == 1) {
                      ...
                      this.api.postAPI(environment.API_URL + "api/auth/authentications", { user_id: res.user_id, user_role_id: res.role_center[0].user_role.id, center_id: (res.role_center[0].center ? res.role_center[0].center.id : ''), 'Unit_name': 'CBIU' }).subscribe((res2) => {
