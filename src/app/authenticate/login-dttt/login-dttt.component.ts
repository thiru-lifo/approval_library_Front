...
@Component({
  selector: 'app-login-dttt',
  templateUrl: './login-dttt.component.html',
  styleUrls: ['./login-dttt.component.scss']
})
export class LoginDtttComponent implements OnInit {
      ...
      this.api.getToken(
        ...
        .subscribe((res) => {
          ...
          if(res.status=='success')
          {
            ...
            localStorage.setItem('APPLOGO','assets/images/dttt-logo.jpg');
            localStorage.setItem('UNITTITLE','DTTT');
            localStorage.setItem('LOGINPAGE','/authenticate/login/dttt');
                  ...
                  this.api.getAPI(environment.API_URL+"access/permissions?user_role_id="+res.role_center[0].user_role.id+'&process_id='+res.process_id).subscribe((res1)=>{
                    if(res1.status==environment.SUCCESS_CODE) {
                      if(res1.data.length==1){
                        ...
                        // For removes CBPM from Transactions
                        ...
                        this.api.postAPI(environment.API_URL+"api/auth/authentications",{user_id:res.user_id,user_role_id:res.role_center[0].user_role.id,center_id:(res.role_center[0].center?res.role_center[0].center.id:''),'Unit_name':'DTTT'}).subscribe((res2)=>{
