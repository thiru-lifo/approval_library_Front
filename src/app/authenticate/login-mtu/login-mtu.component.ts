...
@Component({
  selector: 'app-login-mtu',
  templateUrl: './login-mtu.component.html',
  styleUrls: ['./login-mtu.component.scss']
})
export class LoginMtuComponent implements OnInit {
      ...
      this.api.getToken(
        ...
        .subscribe((res) => {
          ...
          if(res.status=='success')
          {
            ...
            localStorage.setItem('APPLOGO','assets/images/mtu-logo.jpg');
            localStorage.setItem('UNITTITLE','MTU');
            localStorage.setItem('LOGINPAGE','/authenticate/login/mtu');
            ...
            if(res.role_center.length==0)
            {
              this.notification.displayMessage("It seems no previlleges has been set for this account. Please contact administrator");
                ...
                if(res.role_center.length>1) {
                  // this.router.navigateByUrl('/authenticate/role-selection/?id=MTU');
                  ...
                  this.api.getAPI(environment.API_URL+"access/permissions?user_role_id="+res.role_center[0].user_role.id+'&process_id='+res.process_id).subscribe((res1)=>{
                    if(res1.status==environment.SUCCESS_CODE) {
                      if(res1.data.length==1){
                        ...
                        this.api.postAPI(environment.API_URL+"api/auth/authentications",{user_id:res.user_id,user_role_id:res.role_center[0].user_role.id,center_id:(res.role_center[0].center?res.role_center[0].center.id:''),'Unit_name':'MTU'}).subscribe((res2)=>{
